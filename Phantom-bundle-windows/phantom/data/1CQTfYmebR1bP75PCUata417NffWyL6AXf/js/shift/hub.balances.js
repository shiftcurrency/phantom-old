...
//				d1.push([new Date(Date.today().add(i).days()).getTime(), amount]); //here we generate data for chart
    ...
    HUB.adressbook = function(account){
	  /* Fill address book */
	  var aliasses = HUB.startRequest("get_address_book");
	  if (typeof aliasses.result != 'undefined' && aliasses.result[0] != 'false') {
		aliasses = aliasses.result;
		aliasses.forEach(function(alias){
		  $('#contacts').append('<li><a href="#'+alias[0]+'"><span class="pull-right" style="color:#aaa"> [0x'+alias[0].substring(0,3)+' ... '+alias[0].substring(37,40)+']</span>'+alias[1]+'</a></li>');
		});
		$('#contacts > li > a').on('click',function() { 
		  var value = '';
		  if (this.href.indexOf('#') != -1) value = '0x'+$(this).attr('href').substring(1);
		  $('#tx-to').val(value); 
		});
    ...
    HUB.send = function(from, to, amount, pass){
		...
		if (tx_to.indexOf(prefix)==-1) tx_to = prefix+''+tx_to;
		if (tx_from.indexOf(prefix)==-1) tx_from = prefix+''+tx_from;
