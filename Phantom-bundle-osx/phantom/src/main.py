...
update_after_shutdown = False  # If set True then update and restart phantom after main loop ended
...
if config.action == "main":
    ...
    try:
        ...
        print "Can't lock %s file, your Phantom client is probably already running, exiting... (%s)" % (log_file_path, err)
        sys.exit()
...
if config.debug:
    console_log.setLevel(logging.WARNING)  # Display everything to console
else:
    console_log.setLevel(logging.WARNING)  # Display only important info to console
...
class Actions(object):
    ...
    def main(self):
        ...
        print "- Created Phantom Fileserver."
...
actions = Actions()
# Starts here when running phantom.py
