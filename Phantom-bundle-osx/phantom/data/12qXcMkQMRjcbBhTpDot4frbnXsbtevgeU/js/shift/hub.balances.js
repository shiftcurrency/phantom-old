...
var HUB = (function(HUB, $, undefined) {
	...
	HUB.show_balances = function(account, lookup){
		...
		if (confirmed_balance > 0 || pending_balance > 0) {
			...
			bigfloat = BigNumber(confirmed_balance);
			confirmed_balance = bigfloat.toFixed() / 1000000000000000000; 
			
			bigfloat = BigNumber(pending_balance);
			...
			HUB.balance = pending_balance + confirmed_balance;
			window.updateTotal();
		}

    }
 
    HUB.show_txs = function(address){
		if (!address) return;
		
		var error,
			txs = {},
			address = HUB.format_address(address, true), 	
			data = HUB.startRequest("get_transaction_history",'["'+address+'"]'); // [txid, block, sender, recepient, amount]
			
		if (typeof data.error != 'undefined') {
			error = data.error.message;
		} else if (typeof data.result != 'undefined' && data.result instanceof String) {
			error = data.result[1]+".\n"+HUB.runError;
		}
...
//			console.log(txs);
			...
			if (txs.length > 0) $("#datatable").find('tbody').empty()
			txs.forEach(function(tx, i){
				blocknr = tx[1]*1;
				amount = tx[4]*1;
				amount = amount.toFixed() / 1000000000000000000; // Caution: already a bignumber!
				incoming = HUB.activeAccount == HUB.prefix+''+tx[3];
				
				if (incoming) received++;
				else sent++;
				
				tr = '<tr>'
					+ '<td><i class="fa fa-angle-double-'+(incoming ? 'down' : 'up')+'"></i><a class="title" href="#"> '+(incoming ? 'My wallet' : address)+'</a></td>'
					+ '<td>'+(incoming ? tx[2] : tx[3])+'</td>'
					+ '<td>'+amount+'</td>'
					+ '<td>'+(HUB.blocknumber - blocknr)+'</td>'
					+ '<td>x Hrs ago</td>'
					+ '<td>No comment found</td>'
				  + '</tr>';				
				$("#datatable").find('tbody').append($(tr));

//				d1.push([new Date(Date.today().add(i).days()).getTime(), amount]); //here we generate data for chart
				d1[blocknr] = [blocknr, amount];
            });	
			
			/* Update UI */
			$('#transaction_total').attr('data-value',sent + received).text(sent + received);
			$('#transaction_sent').attr('data-value',sent).text(sent);
			$('#transaction_received').attr('data-value',received).text(received);
			$('#number_transactions').attr('data-value',txs.length).text(txs.length);
    ...
    HUB.adressbook = function(account){
	  /* Fill address book pulldown */
	  var address = '',
	  ...
	  if (typeof aliasses.result != 'undefined' && aliasses.result[0] != 'false') {
		...
		aliasses.forEach(function(alias){
		  address = HUB.format_address(alias[0], true);
		  $('#contacts').append('<li><a href="#'+alias[0]+'"><span class="pull-right" style="color:#aaa"> ['+address.substring(0,5)+' ... '+address.substring(37,40)+']</span>'+alias[1]+'</a></li>');
		});
		$('#contacts > li > a').on('click',function() { 
		  if (this.href.indexOf('#') != -1) address = $(this).attr('href').substring(1);
		  address = HUB.format_address(address, true);
		  $('#tx-to').val(address); 
		});
		$('#contacts').prepend('<li><a href="contacts.html">Addressbook</a></li><li role="separator" class="divider"></li>');
	  }	
    }
	
    HUB.send = function(from, to, amount, pass){
		var error,
			data = [{'result':false}],
			tr = '',
			tx_id = '',
		...
		if (tx_to.indexOf(HUB.prefix)==-1) tx_to = HUB.prefix+''+tx_to;
		if (tx_from.indexOf(HUB.prefix)==-1) tx_from = HUB.prefix+''+tx_from;

		if (tx_from == '' || tx_to == '' || isNaN(tx_amount) || tx_amount == 0) error = "Invalid input";	
		else if (tx_from == tx_to) error = "Transaction stopped: Why would you send to yourself?";
		else {
			data = HUB.startRequest("send_transaction",'[{"from": "'+tx_from+'","to": "'+tx_to+'","amount": "'+tx_amount+'","password": "'+tx_pass+'"}]'); 
			if (typeof data.error != 'undefined') {
				error = data.error.message;
			} else if (typeof data.result != 'undefined' && data.result.length > 0) {
				error = data.result[1];
		...
		if (typeof data.result == 'string' || data.result instanceof String) {
			...
			tr = '<tr>'
				+ '<td><i class="fa fa-angle-double-up"></i><a class="title" href="#"> Unknown</a></td>'
				+ '<td>'+tx_to+'</td>'
				+ '<td>'+tx_amount+'</td>'
				+ '<td>-1</td>'
				+ '<td>Now</td>'
			...
			$("#datatable").find('.dataTables_empty').remove(); 
			$(tr).hide().prependTo("#datatable tbody").fadeIn(); 
			
			$("#TxReset").click(); // Empty form
			
			/* Increase txs count */
	  		$('#transaction_total').text(parseInt($('#transaction_total').attr('data-value')) + 1);
			$('#transaction_sent').text(parseInt($('#transaction_sent').attr('data-value')) + 1);
			$('#number_transactions').text(parseInt($('#number_transactions').attr('data-value')) + 1);
			
			/* Decrease balances */			
			$('#balance_unconfirmed').text(parseInt($('#balance_unconfirmed').attr('data-value')) - tx_amount);
		
		} else {
			Site.cmd("wrapperNotification", ["error", error, 5000]);
		} 
 		$("#TxSubmit").removeAttr("disabled readonly");	
    }
	
    $("#TxSubmit").on("click", function(e){
		e.preventDefault();
		if (!$(this).attr('readonly') && !$(this).attr('disabled')) $('form[name="txsend"]').submit();		
    });	
	
	$('form[name="txsend"]').on("submit", function(e) {
		$("#TxSubmit").prop('readonly',true).prop('disabled',true);
		HUB.send(HUB.activeAccount,$("#tx-to").val(),$("#tx-value").val(),$("#tx-password").val());
