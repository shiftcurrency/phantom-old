...
class ActionsPlugin(object):
    ...
    def main(self):
        ...
        icon.items = (
            ...
            "--",
            ("Phantom Twitter", lambda: self.opensite("https://twitter.com/HelloPhantom")),
            ("Phantom Reddit", lambda: self.opensite("http://www.reddit.com/r/phantom/")),
            ("Phantom Github", lambda: self.opensite("https://github.com/HelloPhantom/Phantom")),
            ("Report bug/request feature", lambda: self.opensite("https://github.com/HelloPhantom/Phantom/issues")),
            "--",
    ...
    def getAutorunPath(self):
        return "%s\\phantom.cmd" % winfolders.get(winfolders.STARTUP)
    ...
    def formatAutorun(self):
        ...
        cmd = cmd.replace("start.py", "phantom.py").replace('"--open_browser"', "").replace('"default_browser"', "").strip()
    ...
    def titleAutorun(self):
        if self.isAutorunEnabled():
            return "+Start Phantom when Windows starts"
        else:
            return "Start Phantom when Windows starts"
