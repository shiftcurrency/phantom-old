...
class UiRequest(object):
    ...
    def getPosted(self):
        if self.env['REQUEST_METHOD'] == "POST":
            return dict(cgi.parse_qsl(self.env['wsgi.input'].readline().decode()))
        else:
            return {}
    ...
    def formatError(self, title, message, details=True):
        ...
        if details:
            ...
            details["version_phantom"] = "%s r%s" % (config.version, config.rev)
            ...
            return """
                ...
                * { font-family: Consolas, Monospace; color: #333 }
                ...
                <h1>%s</h1>
                <h2>%s</h3>
                <h3>Please <a href="https://github.com/HelloPhantom/Phantom/issues" target="_blank">report it</a> if you think this an error.</h3>
                <h4>Details:</h4>
                <pre>%s</pre>
