...
def processNameOp(domain, value):
    ...
    try:
        ...
        return False
    if "phantom" not in data:
        print "No phantom in ", data.keys()
        return False
    if not isinstance(data["phantom"], dict):
        print "Not dict: ", data["phantom"]
        return False
    ...
    for subdomain, address in data["phantom"].items():
...
if not os.path.isfile(config_path):  # Create sample config
    open(config_path, "w").write(
        json.dumps({'site': 'site', 'phantom_path': '/home/phantom', 'privatekey': '', 'lastprocessed': 223910}, indent=2)
    )
    print "Example config written to %s" % config_path
    sys.exit(0)

config = json.load(open(config_path))
names_path = "%s/data/%s/data/names.json" % (config["phantom_path"], config["site"])
os.chdir(config["phantom_path"])  # Change working dir - tells script where Zeronet install is.

# Parameters to sign and publish
command_sign_publish = [sys.executable, "phantom.py", "siteSign", "--publish", config["site"], config["privatekey"]]
if sys.platform == 'win32':
...
# processBlock(223911) # Testing phantomwork.bit
