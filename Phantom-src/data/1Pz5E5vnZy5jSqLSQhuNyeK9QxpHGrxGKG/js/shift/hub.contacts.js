var HUB = (function(HUB, $, undefined) {
    ...
    HUB.loadContacts = function(){
		...
		aliasses.forEach(function(contact){	
			HUB.generate(HUB.format_address(contact[0]), contact[1], false);
		});
    }
	...
	HUB.remove = function(address) {
		...
		var data = HUB.startRequest("del_address_book",'["'+HUB.format_address(address, true)+'"]');
    ...
    HUB.create = function(address, alias){
		...
		if (address === '' || alias === '') {
			...
			if (address.length != 42) error = "Invalid wallet adress provided";
			else {
				...
				if (typeof data.error != 'undefined') {
					...
					address = HUB.format_address(address, false);
